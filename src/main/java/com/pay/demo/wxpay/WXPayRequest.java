...
import static com.pay.demo.wxpay.WXPayConstants.USER_AGENT;
...
public class WXPayRequest {
    ...
    private String request(String urlSuffix, String uuid, String data, int connectTimeoutMs, int readTimeoutMs, boolean useCert, boolean autoReport) throws Exception {
        ...
        try {
            String result = requestOnce(config.getWXPayDomain(), urlSuffix, uuid, data, connectTimeoutMs, readTimeoutMs, useCert);
            return result;
        }
        catch (UnknownHostException ex) {  // dns 解析错误，或域名不存在
            exception = ex;
            WXPayUtil.getLogger().warn("UnknownHostException for domainInfo {}", ex.getMessage());
        }
        catch (ConnectTimeoutException ex) {
            WXPayUtil.getLogger().warn("connect timeout happened for domainInfo {}", ex.getMessage());
        }
        catch (SocketTimeoutException ex) {
            exception = ex;
            WXPayUtil.getLogger().warn("timeout happened for domainInfo {}", ex.getMessage());
        }
        catch (Exception ex) {
            exception = ex;
        }
